# WebSecurity.com - Professional Tech Store Clone

A modern, glassmorphism-styled e-commerce website inspired by StarTech, built with Next.js 14, TypeScript, and TailwindCSS.

## 🌟 Features

### ✨ Design & UI
- **Glassmorphism Effects**: Modern glass-like UI components with backdrop blur
- **Responsive Design**: Mobile-first approach with seamless desktop experience
- **Animated Components**: Smooth hover effects and transitions using Framer Motion
- **Professional Dashboard**: Complete admin panel for store management

### 🛍️ E-commerce Features
- **Product Categories**: Security Systems, Laptops, Desktops, Gaming PCs, Mobile Phones, Accessories
- **Advanced Search**: Real-time product search functionality
- **Shopping Cart**: Add to cart with quantity management
- **User Authentication**: Login/Register system
- **Product Management**: Admin dashboard for inventory control

### 🔧 Technical Features
- **Next.js 14**: Latest React framework with App Router
- **TypeScript**: Type-safe development
- **TailwindCSS**: Utility-first CSS framework
- **Framer Motion**: Smooth animations and transitions
- **Lucide Icons**: Beautiful, consistent iconography

## 🚀 Getting Started

### Prerequisites
- Node.js 18+ 
- npm or yarn

### Installation

1. **Clone the repository**
   ```bash
   git clone <repository-url>
   cd Star_Tech_Clone
   ```

2. **Install dependencies**
   ```bash
   npm install
   ```

3. **Run the development server**
   ```bash
   npm run dev
   ```

4. **Open your browser**
   Navigate to [http://localhost:3000](http://localhost:3000)

## 📁 Project Structure

```
├── app/                    # Next.js 14 App Router
│   ├── dashboard/         # Admin dashboard pages
│   ├── globals.css        # Global styles with glassmorphism
│   ├── layout.tsx         # Root layout
│   └── page.tsx          # Home page
├── components/            # Reusable components
│   ├── ui/               # UI components (GlassCard, etc.)
│   └── Header.tsx        # Main navigation
├── public/               # Static assets
└── styles/              # Additional styling
```

## 🎨 Design System

### Glassmorphism Classes
- `.glass` - Basic glass effect with backdrop blur
- `.glass-dark` - Dark variant for headers/overlays
- `.glass-card` - Interactive card with hover effects
- `.glass-button` - Button with glass styling
- `.gradient-text` - Gradient text effect
- `.neon-glow` - Subtle glow effect

### Color Palette
- **Primary**: Blue gradient (blue-400 to blue-600)
- **Secondary**: Purple accent (purple-400)
- **Background**: Dark gradient (slate-900 to purple-900)
- **Glass**: Semi-transparent white overlays

## 📱 Pages & Features

### 🏠 Home Page (`/`)
- Hero section with animated text
- Featured product categories
- Sample products showcase
- Company statistics

### 📊 Admin Dashboard (`/dashboard`)
- **Overview**: Revenue, orders, products, users statistics
- **Products**: Product management with CRUD operations
- **Orders**: Order tracking and management
- **Users**: User account management
- Real-time data visualization

## 🛠️ Technologies Used

- **Framework**: Next.js 14 with App Router
- **Language**: TypeScript
- **Styling**: TailwindCSS with custom glassmorphism utilities
- **Animation**: Framer Motion
- **Icons**: Lucide React
- **Notifications**: React Hot Toast
- **State Management**: Zustand (ready for implementation)
- **Authentication**: NextAuth.js (configured)

## 🎯 Sample Products

### Security Systems
- Professional Security Camera System - $299.99

### Laptops  
- Gaming Laptop RTX 4070 - $1,299.99

### Categories Available
- Security Systems (150+ products)
- Laptops (200+ products) 
- Desktops (180+ products)
- Gaming PCs (120+ products)
- Mobile Phones (300+ products)
- Accessories (500+ products)
- Networking (80+ products)
- Software (60+ products)

## 🚀 Deployment

### Build for Production
```bash
npm run build
npm start
```

### Deploy to Vercel
```bash
npx vercel
```

## 🔮 Future Enhancements

- [ ] Complete shopping cart functionality
- [ ] User authentication system
- [ ] Payment integration
- [ ] Product reviews and ratings
- [ ] Advanced filtering and sorting
- [ ] Wishlist functionality
- [ ] Real-time notifications
- [ ] Multi-language support

## 📄 License

This project is created for educational purposes as a website clone demonstration.

## 🤝 Contributing

1. Fork the repository
2. Create your feature branch (`git checkout -b feature/AmazingFeature`)
3. Commit your changes (`git commit -m 'Add some AmazingFeature'`)
4. Push to the branch (`git push origin feature/AmazingFeature`)
5. Open a Pull Request

---

**WebSecurity.com** - Leading Security & Tech Solutions with Modern Glassmorphism Design
